...
##textplot_scale1d() used to visulise scale of documents and words typically from wordfish method. this reveals patterns or relationship of words based on position in the dimensions
```
...
##this creates a new variable called features, by extracting all the word from dfm_newstweets_results

betas <- dfm_newstweets_results[["beta"]]
##this creates a new variable called beta, extracting all the scaling position of the words.
...
```{r}
##since the code before the exercise similar to row 97 we can start from there
